...
int main()
{
	...
	printf("pid arriving_time  priority  response_time  waiting_time  turn_around_time\n");
	for(int i=0;i<np;i++)
	{
		printf("%d\t%d\t\t%d\t%d\t\t%d\t\t%d\n",p[i].pid,p[i].arrival_t,p[i].priority,p[i].response_t,p[i].waiting_t,p[i].turn_around_t);
	}
	return 0;
}
...
bool sort_prio(process* a,process* b)                  //sort according to priority
{ 
...
void ROUND_ROBIN()
{
	queue<process*> p_queue;        // process queue
	...
	while(true)
	{
		...
		{  
		//	printf("%d\n",indexNotYetPushed);
			p[indexNotYetPushed].quantum=TIMESLICE;
		...
		if(!p_queue.empty()  && p_queue.front()->quantum==0)
		{
				if(p_queue.front()->timeLeft!=0)
				{
					p_queue.front()->quantum=TIMESLICE;
					p_queue.push(p_queue.front());
					p_queue.pop();
				}
		}
		...
		//		printf("process %d terminated\n",i_queue.front()->pid);
			...
			{
				i_queue.front()->quantum=TIMESLICE;
		...
		//		printf("process %d terminated\n",o_queue.front()->pid);
			...
			{
				o_queue.front()->quantum=TIMESLICE;
		...
		if(!p_queue.empty())
		{
			...
			if(p_queue.front()->quantum!=0)
			{
				p_queue.front()->quantum--;
			}
